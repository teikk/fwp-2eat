...
<div class="fwpr-order-products">
	<h2 class="fwpr-order-products__title"><?php _e('Zakończone diety','fwpr'); ?></h2>
		...
		if( $rows ) {
			foreach ($rows as $key => $row) {
				...
				if( $dates ) {
					...
					$lastDate = end( $dates );
					if( fwpr_getTimestamp($lastDate['date']) < fwpr_getTimestamp($current_date) ) {
					?>
			...
			_e('Nie masz zakończonych zamówień','fwpr');
		}
	?>
	</div>
</div>